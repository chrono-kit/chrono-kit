...
class ETS_MNM(Model):
    ...
    def __smooth_level(self , lprev , error , seasonal): ##done
        """Calculate the level"""

        self.level = torch.multiply(lprev, torch.add(1, torch.multiply(alpha, error)))

    def __smooth_error(self , y, lprev, error, seasonal):  

        """Calculate error"""

        self.error = torch.divide(torch.sub(y, torch.multiply(lprev, torch.add(1, torch.multiply(alpha, error)))), torch.multiply(lprev, torch.add(1, torch.multiply(alpha, error))))

    def __smooth_seasonal(self, seasonal,error,lprev): ##done
        """Calculate the level"""

        seasonal = torch.multiply(self.seasonals[-self.seasonal_periods], torch.add(1, torch.multiply(gama, error)))
    ...
    def predict(self, h, return_confidence=False): ###???????????????
        ...
        if return_confidence:
            ...
            self.forecast = torch.multiply(self.level,self.seasonals[-self.seasonal_periods])
            ...
            for i in range(1,h):
                ...
                step_forecast = self.level + i * self.seasonal   ###??????
            ...
            self.forecast = torch.multiply(self.level,self.seasonals[-self.seasonal_periods])  ###????
            for i in range(1,h):
                self.forecast = torch.cat((self.forecast, self.level,self.seasonal))  ###?????????
