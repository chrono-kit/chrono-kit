...
def MSTL(data, seasonal_periods, method="add", degrees=None, robust=True, refine_iterations=1,
    ...
    *refine_iterations Optional[int]: Number of iterations when refining the seasonal components. Default=1.
    *outer_iterations Optional[int]: Number of iterations in the outer loop of each STL operaton. Default = 10.
    *inner_iterations Optional[int]: Number of iterations in the inner loop of each STL operation. Defaut = 2.
    *post_smoothing Optional[bool]: Whether to perform a final LOESS smoothing for trend and seasonal components for each STL. Default = False.
    ...
    **trend_degrees (array_like): Degree used on each STL for the trend component. Will override the argument *degree for the trend component.
    **seasonal_degrees (array_like): Degree used on each STL for the seasonal component. Will override the argument *degree for the seasonal component.
    **low_pass_degrees (array_like): Degree used on each STL for the low pass filtering. Will override the argument *degree for the low pass filtering.
    ...
    *remainder (np.ndarray): Remainder component of the data
    ...
    allowed_kwargs = ["trend_windows", "seasonal_windows", "low_pass_windows",
                      "trend_degrees", "seasonal_degrees", "low_pass_degrees"]
    ...
    for iter in range(refine_iterations):
