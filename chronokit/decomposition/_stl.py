...
def LOESS(data, window_size, degree=1, robustness_weights=None):
    ...
    *y_smoothed (np.ndarray): Smoothed values as a result of LOESS.
    ...
    def loess(x, y, xi, degree=1, robustness_weights=None, q=None):
        ...
        *xi (int): Index to assign weight == 1. I.e; the point where the smoothing result matters the most
        *degree (int): Degree of the polynomial p[x] = y
        *robustness_weights (np.ndarray): Robustness weights for outliers.
        *q (int): Distance for weighting data points. All x s.t distance(x-xi) >= q will be assigned weights=0. Must be odd.
        """
        ...
        #Calculate weights by the tricube function, making sure that data points farther than q are given weight 0
    ...
    #Calculate the half of the window size to use for weighting data points in loess(used as q), must be odd
...
def __inner_loop(y, trend, weights, seasonal_period, seasonal_degree=1, trend_degree=1, low_pass_degree=1,
               seasonal_smoothing=None, trend_smoothing=None, low_pass_smoothing=None, n_iterations=2):
    """
    ...
    *n_iterations Optional[int]: Number of iterations for the inner loop.
    ...
    for i in range(n_iterations):
        ...
        for i in range(seasonal_period):
            #Get each cycle-subseries from detrended data and store them in the cycle_subseries list
        ...
        for ind, subseries in enumerate(cycle_subseries):
            ...
            #Apply scipy.lstsq to extrapolate values for the endpoints of the cycle subseries based on LOESS results
...
def STL(data, seasonal_period, method="add", degree=1, robust=True, outer_iterations=10, inner_iterations=2, post_smoothing=False, show=False,
    ...
    *post_smoothing Optional[bool]: Whether to perform a final LOESS smoothing for trend and seasonal components post operation. Default = False.
    ...
    **trend_degree (int): Degree used on LOESS for the trend component. Will override the argument *degree for the trend component.
    **seasonal_degree (int): Degree used on LOESS for the seasonal component. Will override the argument *degree for the seasonal component.
    **low_pass_degree (int): Degree used on LOESS for the low pass filtering. Will override the argument *degree for the low pass filtering.
    ...
    *remainder (np.ndarray): Remainder component of the data
    ...
    #If low pass window is not given,take it as the least odd integer greater than or equal to seasonal period
    ...
    assert(low_pass_window % 2 == 1 and low_pass_window >= seasonal_period), "Window size for low pass filtering must be odd and greater than or equal to the seasonal period"
