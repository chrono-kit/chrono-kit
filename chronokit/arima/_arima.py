...
class ARIMA(Model):
    def __init__(self, data, p, d, q):
        ...
        *d (int): Order of differencing. Only order d=0 and d=1
                  are supported as of v1.0.x
        *q (int): Order of the moving average part
        ...
        assert self.p >= 0 and isinstance(p, int), "p must be an integer >= 0"
        assert self.d >= 0 and isinstance(d, int), "d must be an integer >= 0"
        assert self.q >= 0 and isinstance(q, int), "q must be an integer >= 0"

        # ARIMA(0,d,0) models are not implemented, as they correspond to just
        # white noise or random walks with/without drift
        if self.p == 0 and self.q == 0:
            raise NotImplementedError("ARIMA models with p = 0 and q = 0 are not implemented")

        # Differencing of order grater than 1 for ARIMA models
        # are not implemented as of v1.0.x
        if self.d > 1:
        ...
        self.__model = {
            ...
            (True, True, True): "ARIMA",
        }[bool(self.p), bool(self.d), bool(self.q)]
        ...
        # Define self.fitted to assert whether the model was fitted
    ...
    def fit(self):
        ...
        # Define sse(Sum of Squared Error) function for possibly
        # estimating phi and theta if self.q != 0
        def sse(init_values):
        ...
        if "MA" in self.__model:
            ...
            # If the model has an AR part, use phi parameters
            # that were estimated from yule-walker as initial values
        ...
        for i in range(len(self.data)):
            ...
            if self.__model in ["AR", "ARI"]:
                ...
                if i < self.p:
                    ...
                    # AR(p) => y(hat)_t = phi(1)*y_t-1 +
                    # phi(2)*y_t-2 ... + phi(p)*y_t-p
            ...
            # If the model is a MA model
                ...
                if self.q < len(self.errors):
                    ...
                    # MA(q) => y(hat)_t = theta(1)*e_t-1 +
                    # theta(2)*e_t-2 ... + theta(p)*e_t-p
            ...
            # If the model is an ARIMA model
                ...
                if i < self.p:
                    ...
                    # ARMA(p,q) => y(hat)_t = phi(1)*y_t-1 + ... +
                    # phi(p)*y_t-p + theta(1)*error_t-1 + .... +
                    # theta(p)*error_t-p
                    y_hat = torch.sum(self.phi * self.data[i - self.p : i].flip(0))
                    if self.q < len(self.errors):
                        y_hat += torch.sum(
                            self.theta * torch.tensor(self.errors[-self.q :]).flip(0)
                        )
    ...
    def predict(self, h):
        ...
        *forecast (array_like): Predicted values
        ...
        assert h > 0 and isinstance(h, int), "Provide h as an integer greater than 0"
        ...
        if self.__model == "ARI":
            ...
            # If AR model with differencing change parameters
            # to use for forecasting
            # Forecast equations for AR(p) will resemble AR(p+1) with
            # parameters [(1+phi_1) - (phi_1-phi_2) - ... -
            # (phi_p-1 - phi_p) - phi_p]
            ...
            # Use the last p values of fitted data and last q
            # values of residuals for forecasting
            fit_vals = self.data[-self.p :]
            fit_errs = torch.tensor(self.errors[-self.q :])

        elif self.__model == "ARIMA":
            # Use the last p+d values of original data and
            # last q values of residuals for forecasting
        ...
        for i in range(h):
            # Apply respective forecast equations for each model
            # and store it in forecast array
            if self.__model == "ARI":
            ...
            # Add the point forecast to the last index of the array
            # used for forecasting the next step
            if self.__model in ["IMA", "MA"]:
                fit_errs = torch.cat((fit_errs[1:], torch.tensor([0.0])))
        ...
        # If MA(q) model with differencing, add the predicted values
        # to the last value of the original data to get actual forecasts
        if self.__model == "IMA":
