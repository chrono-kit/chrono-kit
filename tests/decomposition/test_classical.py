...
class TestClassical(unittest.TestCase):
    def test_passengers_add(self):
        trend, seasonal, remainder = classical_decomposition(
            data=passengers.values, seasonal_period=12, method="add"
        )
        ...
        s_diff = np.nanmean(np.sqrt(np.square(seasonal - sm_results.seasonal))) / np.nanstd(
            seasonal
        )
    ...
    def test_temp1(self):
        trend, seasonal, remainder = classical_decomposition(
            data=temp1.values, seasonal_period=12, method="add"
        )
    ...
    def test_temp2(self):
        trend, seasonal, remainder = classical_decomposition(
            data=temp2.values, seasonal_period=12, method="add"
        )
    ...
    def test_sunspots(self):
        trend, seasonal, remainder = classical_decomposition(
            data=sunspots.values, seasonal_period=12, method="add"
        )
