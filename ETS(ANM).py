...
class ETS_ANM(Model):
    ...
    def __smooth_error(self, y, lprev,seasonal):  
        ...
        self.error = torch.sub(y,torch.multiply(seasonal,lprev))

    def __smooth_seasonal(self, seasonal,error,lprev): ##done
        """Calculate the level"""

        seasonal = torch.add(self.seasonals[-self.seasonal_periods], torch.divide(torch.multiply(self.gamma, error),lprev))
