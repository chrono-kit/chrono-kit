...
class ETS_ANA(Model):
    ...
    def __smooth_level(self,lprev,error): ##done
        """Calculate the level"""
    ...
    def __smooth_error(self, y, lprev,seasonal):  # What is the equation??
        ...
        self.error = torch.sub(torch.sub(y, lprev),seasonal)

    def __smooth_seasonal(self, seasonal,error): ##done
        """Calculate the level"""

        seasonal = torch.add(self.seasonals[-self.seasonal_periods], torch.multiply(self.gamma, error))
    ...
    def fit(self):                 ##DONE
        ...
        for index, row in enumerate(self.dep_var):
            ...
            if index == 0:
                ...
                self.__smooth_level(self.level,self.error)
                self.seasonals = torch.cat((self.seasonals, torch.tensor([seasonal])))
